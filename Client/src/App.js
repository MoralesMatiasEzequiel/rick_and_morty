...
function App() {
   ...
   // const login = (userData) => { //Con esta funcion le mandamos info al back-end.
   //    const { email, password } = userData;
   //    const URL = 'http://localhost:3001/rickandmorty/login/';  //Esta URL es un objeto. Por eso mas adelante hacemos un destructuring de "data". "data" es una propiedad de este objete, que a la vez es un objeto tambien.
   //    axios(URL + `?email=${email}&password=${password}`)  //Aqui concatenamos "email" y "password" a la URL de nuestro servidor
   //    .then(({ data }) => { //Este destructuring de "data" es similar a: ".then(response => response.data).then((data) => {...}"   
   //       const { access } = data;
   //       setAccess(access);
   //       access && navigate('/home');
   //    });
   ...
   //       axios(`http://localhost:3001/rickandmorty/character/${id}`).then(({ data }) => { //Este es nuestro nuevo servidor (el nuestro). Y de esta manera conectamos el front-end con el back-end.     //Ex url: `${URL_BASE}/${id}?key=${API_KEY}`
   //       //Ex ex ex axios(`https://rickandmortyapi.com/api/character/${id}`).then(({ data }) => {   //Todo esto es similar a lo que haciamos con AJAX. Aqui le pasamos entre () la url del servidor. El ".then()" recibe la respuesta de la API. Es similar al "$.get()=>{}". Esto es PROMESAS. Con "{ data }" estamos haciendo destructuring, porque me quedo con "data" de todo el objeto gigante que devuelve axios.
   //       if (data.name) {
   //          setCharacters((oldChars) => [...oldChars, data]);
   //       } else {
